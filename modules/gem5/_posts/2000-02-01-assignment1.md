...
3. **sieve-config.py (10 points)** This file will be checked to make sure it executes correctly in gem5. It should take no parameters, and it should execute the `sieve` binary generated by `g++` with the command above. If you need any additional python files be sure to upload them as well (e.g., `caches.py`)
