...
- [Assignment 5 -- Due 11:59 pm (PST) Friday, March 11th 2022](#assignment-5----due-1159-pm-pst-friday-march-11th-2022)
...
We will take a very simple application, summing the values in an array, and see how if you are not careful how you parallelize the application that the performance will be quite poor.
...
`assignment5-template/parallel.cpp`
  - The 6 different algorithms. You can compile this to generate `parallel` binaries.
`assignment5-template/run.py`
  - The gem5 run script
`assignment5-template/parallel-x86`
  - The `parallel` binary to run on real hardware.
`assignment5-template/parallel-x86-gem5`
  - The `parallel` binary to use with gem5. Same as `parallel-x86` except for the gem5-specific special instructions around the region of interest.
`assignment5-template/my_mesi/`
  - The files for the coherence protocol configuration
`assignment5-template/gem5-x86`
  - The gem5 binary.
